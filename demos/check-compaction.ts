...
async function compact(
    ...
    const res = await completeSimple(model, ctx, {
        apiKey: API_KEY,
        maxTokens: MAX_SUMMARY_TOK,
    });
    ...
    try {
        summaryTokens = await tokenize(MODEL_ID, summary, API_KEY);
    } catch {
        summaryTokens = Math.ceil(summary.length / 3.2);
    }
...
async function main() {
    ...
    console.log(
        `Reported window: ${REPORTED_WINDOW} tokens  (×${SAFETY_FACTOR})`,
    );
    console.log(
        `Max summary:     ${MAX_SUMMARY_TOK} tokens  (input budget: ~${Math.round(MAX_INPUT_CHARS / 1000)}k chars)`,
    );
    console.log(
        `Safe budget:     ${SAFE_BUDGET} tokens  (40% of window — exceed this → immediate re-compact)`,
    );
    console.log();

    const entries = loadSession(sessionPath);
    const compacts = entries
        .map((e, i) => ({ ...e, _idx: i }))
    ...
    for (let r = 0; r < compacts.length; r++) {
        ...
        console.log(
            `Stored compaction: tokensBefore=${comp.tokensBefore?.toLocaleString()}, summary=${comp.summary?.length ?? 0} chars`,
        );
        ...
        console.log(
            `Input slice:     ${msgs.length} msgs, ${inputChars.toLocaleString()} chars (~${inputTok} tokens, ${pct(inputTok, REPORTED_WINDOW)} of window)`,
        );
        console.log(`                 ${bar(inputTok, REPORTED_WINDOW)}`);
        console.log(`  Calling API...`);

        const { summary, summaryTokens } = await compact(
            msgs,
            prevStoredSummary,
        );
        ...
        console.log(
            `Our summary:     ${summary.length} chars → ${summaryTokens} tokens  (${pct(summaryTokens, REPORTED_WINDOW)} of window)`,
        );
        console.log(`                 ${bar(summaryTokens, REPORTED_WINDOW)}`);
        console.log(
            `Stored summary:  ${comp.summary?.length ?? 0} chars → ~${storedTokens} tokens`,
        );

        const pass = summaryTokens <= SAFE_BUDGET;
        console.log(
            `Budget check:    ${pass ? "✓ PASS" : "✗ FAIL"}  (budget: ${SAFE_BUDGET} tokens)`,
        );
        if (!pass) {
            console.log(
                `  ⚠  Over by ${summaryTokens - SAFE_BUDGET} tokens. Post-compaction context will be ≥${summaryTokens + 3000} tokens,`,
            );
            console.log(
                `     which is ${pct(summaryTokens + 3000, REPORTED_WINDOW)} of the window. Next compaction fires almost immediately.`,
            );
        }
        console.log();
