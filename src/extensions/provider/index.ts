...
import { FEATHERLESS_MODELS, MODEL_META } from "./models.js";
...
export function parseToolCallTags(text: string): ParsedContent {
  ...
  while (searchFrom < text.length) {
    ...
    const useToolCall =
      toolCallIdx !== -1 && (funcCallIdx === -1 || toolCallIdx <= funcCallIdx);
    const useFuncCall =
      funcCallIdx !== -1 && (toolCallIdx === -1 || funcCallIdx < toolCallIdx);
    ...
    if (useToolCall) {
      ...
      if (closeIdx === -1) {
        closeIdx = text.indexOf("</functioncall>", openIdx + openTag.length);
      ...
      if (closeIdx !== -1) {
        ...
        if (jsonParsed) {
          ...
          if (nameMatch) {
            ...
            toolCalls.push({ name: nameMatch[1].trim(), arguments: args });
          } else {
            textBefore += text.slice(openIdx, closeIdx + actualCloseLen);
          }
        }
    ...
    try {
      ...
      function processContent(delta: string) {
        if (inTag) {
          ...
          for (const ct of closeTags) {
            ...
            if (closeIdx !== -1) {
              ...
              const remainder = tagBuffer.slice(closeIdx + ct.length);
              inTag = false;
              tagBuffer = "";
        ...
        if (earliestIdx !== -1) {
          ...
          // Enter tag mode
          const afterTag = pendingText.slice(earliestIdx + matchedOpen.length);
          pendingText = "";
          inTag = true;
          tagOpen = matchedOpen;
          tagBuffer = "";
      ...
      for await (const chunk of openaiStream) {
        ...
        if (reasoning) {
          ...
          (output.content[thinkingIndex] as ThinkingContent).thinking +=
            reasoning;
          stream.push({
            type: "thinking_delta",
            contentIndex: thinkingIndex,
        ...
        if (choice.delta.tool_calls) {
          ...
          for (const tc of choice.delta.tool_calls) {
            ...
            if (!entry) {
              entry = {
                id: tc.id ?? `call_${Date.now()}_${toolCallCount++}`,
                name: "",
                arguments: "",
              };
    ...
    handler: async (_args, ctx) => {
      try {
        ...
        const classBuckets = new Map<
          string,
          { models: typeof FEATHERLESS_MODELS; concurrencyCost: number }
        >();
        ...
        for (const [modelClass, bucket] of classBuckets) {
          ...
          if (!entries || entries.length === 0) {
            lines.push(`[cc:${cc}] ${modelClass} -- No data -- ${names}`);
            continue;
          }
          ...
          const avg24h =
            last24.reduce((sum, e) => sum + e.value, 0) / last24.length;
        ...
        for (const [modelClass, entries] of Object.entries(errorRates)) {
